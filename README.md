...
`/stage2` 是第二阶段 v3 的 run 创建、真实浏览器执行、复盘、人工处理和报告入口。只访问 `http://localhost:4173` 时，可能会看到第一阶段或兼容首页。

如果端口 `4173` 已被旧实例占用，优先先访问 `http://localhost:4173/stage2` 确认现有服务是否可复用。需要在 Windows 上查出并结束旧进程时，可执行：
...
- [测试手册第二阶段 v3](docs/测试手册第二阶段v3.md)：日常运行第二阶段 v3 run、连接真实浏览器、复盘、进入下一轮和生成报告。
